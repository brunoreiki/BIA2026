...
class locallib {
    ...
    /**
     * @var number Lower bound for lucidity index.
     */
    public const LUCIDITYINDEXMIN = -100;
    /**
     * @var number Upper bound for lucidity index.
     */
    public const LUCIDITYINDEXMAX = 100;
    ...
    public static function get_lucidity_indicators_for_attempt($qubaorid, $flags = 65535) {
            ...
            foreach ($quba->get_attempt_iterator() as $qa) {
                ...
                if ($subcategory !== null) {
                    ...
                    if ($subcategory->answerclass->correctness === 'incorrect') {
                        $attemptdata->nwrong++;
                        $isunexpectederror = $subcategory->answerclass->name === 'unexpectederror';
                        if ($flags & (static::INDICATOR_IMPRUDENCE | static::INDICATOR_LUCIDITY)) {
                            ...
                            if (preg_match('/([0-9.,]+)\s*%/', $subcategory->certaintylevel->percentage, $matches)) {
                                ...
                                $wrongcertaintysum += $percent;
                                if ($isunexpectederror) {
                                    $lucidityscore -= $percent;
                                } else {
                                    $lucidityscore += 100.0 - $percent;
                                }
                            }
                        }
                        $hasunexpectederror = $hasunexpectederror || $isunexpectederror;
                    } else if ($subcategory->answerclass->correctness === 'correct') {
                        $attemptdata->nright++;
                        $isunsureknowledge = $subcategory->answerclass->name !== 'sureknowledge';
                        if ($flags & (static::INDICATOR_CONFIDENCE | static::INDICATOR_LUCIDITY)) {
                            $matches = null;
                            if (preg_match('/([0-9.,]+)\s*%/', $subcategory->certaintylevel->percentage, $matches)) {
                                $percent = (float) $matches[1];
                                $rightcertaintysum += $percent;
                                if ($isunsureknowledge) {
                                    $lucidityscore -= 100.0 - $percent;
                                } else {
                                    $lucidityscore += $percent;
                                }
                        ...
                        $hasunsureknowledge = $hasunsureknowledge || $isunsureknowledge;
                    } else {
                        $attemptdata->nrandom++;
                    }
                } else {
        ...
        if ($flags & static::INDICATOR_LUCIDITY) {
            if ($attemptdata->nright > 0 || $attemptdata->nwrong > 0) {
                $attemptdata->lucidityindex = round($lucidityscore / ($attemptdata->nright + $attemptdata->nwrong));
