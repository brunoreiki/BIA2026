    ...
    public function summarise_usage(question_usage_by_activity $quba, question_display_options $options) {
        ...
        if ($reportcertaintyquizzesplugin !== null) {
            ...
            if (count($attemptsonotherquizzes) > 1) {
                ...
                $linktoreport = new moodle_url('/report/certaintyquizzes/index.php', [
                        'courseid' => $oneattempt->course,
                        'report' => 'studentsalongcourse',
                        'userid' => $oneattempt->userid,
                ]);
            }
        }
        ...
        $oursummary .= $OUTPUT->render_from_template(locallib::COMPONENT . '/attemptindicators', [
                ...
                'ndeclaredignorance' => $attemptindicators->nrandom,
                'nnotanswered' => $attemptindicators->nnotanswered,
                'nnocertainty' => $attemptindicators->nnocertainty,
                ...
                'showluciditydisclaimer' => $nquestions < 10,
                'linktoreport' => $linktoreport,
        ]);

        // If there are too few questions, expected trend does not make any sense.
        $doexpectedtrend = $nquestions >= 10;

        if ($attemptindicators->nright + $attemptindicators->nwrong + $attemptindicators->nrandom > 0) {
            ...
            if ($doexpectedtrend) {
                ...
                $objectivesum = array_sum(array_map('array_sum', $objectives));
                $coef = ($attemptindicators->nwrong + $attemptindicators->nright) / $objectivesum * 0.5;
            ...
            if ($doexpectedtrend) {
                $toplegendcontext->expectedtrend = [
                        'color' => $expectedtrendcolor,
                        'helpicon' => $OUTPUT->help_icon('expectedtrend', locallib::COMPONENT),
                ];
            } else {
                $toplegendcontext->expectedtrend = false;
            }
            ...
            foreach (answerclass::get_classes() as $answerclass) {
                $class = new stdClass();
                $class->display = $answerclass->nsubcategories > 0;
                $class->spacepercentage = $answerclass->nsubcategories * round(100.0 / $ncategories);
                $class->numanswers = array_sum(array_slice($attemptindicators->subcategorysizes, $i, $answerclass->nsubcategories));
                if ($class->numanswers > 1) {
                    $str = $answerclass->name . 'plural';
                } else if ($class->numanswers == 1) {
                    $str = $answerclass->name;
                } else {
                    $str = 'no' . $answerclass->name;
                }
                $class->name = get_string($str, locallib::COMPONENT);
                $bottomlegendcontext->answerclasses[] = $class;
                $i += $answerclass->nsubcategories;
            }
        ...
        if ($nattempts > 1) {
            ...
            if ($hasdeclaredignorance) {
                $gradeschart->add_series(new \core\chart_series(
                    get_string('declaredignorancerate', locallib::COMPONENT),
            ...
            $luciditychartseries = new chart_series(
                get_string('lucidityindex', locallib::COMPONENT),
            ...
            $luciditychart->add_series(new \core\chart_series(
                get_string('metacognitiveindicator', locallib::COMPONENT),
        ...
        $PAGE->requires->js_call_amd(locallib::COMPONENT . '/chart_postprocessing', 'postprocessing', [
                array_column($attemptsdata, 'chartshape'),
                $hasdeclaredignorance,
                $doexpectedtrend,
                [
                        'min' => locallib::LUCIDITYINDEXMIN,
                        'max' => locallib::LUCIDITYINDEXMAX,
                ],
        ]);
