...
class studentsalongcourse extends report {
    ...
    public function print_report() {
        ...
        if ($this->selectedgroupid > 0) { // Group selected.
            $userids = array_values($this->groups[$this->selectedgroupid]['members']);
            [ $prevlink, $nextlink ] = $this->get_prev_and_next_links($this->groups, $this->selectedgroupid, 'g');
            echo \html_writer::div($prevlink . $this->groups[$this->selectedgroupid]['name'] . $nextlink, 'h4 text-center my-2');
        } else if ($this->selecteduserid == -1) { // All users.
            $userids = array_keys($this->users);
        } else {
            $userids = [ $this->selecteduserid ];
            [ $prevlink, $nextlink ] = $this->get_prev_and_next_links($this->users, $this->selecteduserid, 'u');
            echo \html_writer::div($prevlink . $this->users[$this->selecteduserid]['name'] . $nextlink, 'h4 text-center my-2');
        }
        ...
        $PAGE->requires->js_call_amd(locallib::COMPONENT . '/studentsalongcourse', 'postprocess', [
                ...
                $this->attempttype,
                [
                        'min' => \qbehaviour_certaintywithstudentfbdeferred\locallib::LUCIDITYINDEXMIN,
                        'max' => \qbehaviour_certaintywithstudentfbdeferred\locallib::LUCIDITYINDEXMAX,
                ],
        ]);
    ...
    protected function get_prev_and_next_links($usersorgroups, $currentid, $paramprefix) {
        global $PAGE;
        reset($usersorgroups);
        $previous = null;
        while (key($usersorgroups)) {
            if (key($usersorgroups) == $currentid) {
                break;
            } else {
                $previous = key($usersorgroups);
            }
            next($usersorgroups);
        }
        next($usersorgroups);
        $next = key($usersorgroups) ?: null;
        if ($previous) {
            $previousurl = $PAGE->url;
            $previousurl->param('users', $paramprefix . $previous);
            $title = get_string('previous') . ' (' . $usersorgroups[$previous]['name'] . ')';
            $prevlink = \html_writer::link($previousurl, '<i class="fa fa-lg fa-caret-left"></i>', [
                    'class' => 'btn p-1 mx-3 align-text-top',
                    'title' => $title,
                    'aria-label' => $title,
            ]);
        } else {
        ...
        if ($next) {
            $nexturl = $PAGE->url;
            $nexturl->param('users', $paramprefix . $next);
            $title = get_string('next') . ' (' . $usersorgroups[$next]['name'] . ')';
            $nextlink = \html_writer::link($nexturl, '<i class="fa fa-lg fa-caret-right"></i>', [
                    'class' => 'btn p-1 mx-3 align-text-top',
                    'title' => $title,
                    'aria-label' => $title,
            ]);
