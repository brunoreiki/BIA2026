...
require_once($CFG->dirroot.'/course/lib.php');
        ...
        for ($i = 0; $i < $numactivities; $i++) {
            if ($userid != $USER->id &&
                ...
                $activities[$i]->link = $activi// === BEGIN CUSTOM: Redirect activity links to their section (Tiles-friendly) ===
// Build per-activity links to the containing SECTION instead of the activity URL.
// This works with standard formats (Topics/Weeks) and the "Tiles" format.
// If the course format is Tiles, the &section=X param opens the corresponding tile.
try {
    $course = $progress->get_course();
    $courseid = $course->id;
    $formatobj = course_get_format($courseid);
    $formatname = method_exists($formatobj, 'get_format') ? $formatobj->get_format() : (property_exists($course, 'format') ? $course->format : '');

    $modinfo = \get_fast_modinfo($courseid);
    for ($__i = 0; $__i < $numactivities; $__i++) {
        $cmid = $activities[$__i]->id; // course_modules.id
        if (!empty($cmid) && isset($modinfo->cms[$cmid])) {
            $cm = $modinfo->cms[$cmid];
            // Section number is stable across formats; Tiles also respects section=X.
            $sectionnum = isset($cm->sectionnum) ? $cm->sectionnum : (isset($cm->section->section) ? $cm->section->section : null);
            if ($sectionnum !== null) {
                $sectionurl = new \moodle_url('/course/view.php', ['id' => $courseid, 'section' => $sectionnum]);
                $activities[$__i]->link = $sectionurl->out(false);
                if (!empty($activities[$__i]->onclick)) {
                    unset($activities[$__i]->onclick);
                }
            }
        }
    }
} catch (\Throwable $e) {
    // Fail silently: keep original activity links if anything goes wrong.
}
// === END CUSTOM ===
